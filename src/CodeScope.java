...
public abstract class CodeScope {
    ...
    public void addSimpleMessage(String simpleMessage) {
        addOutput(tabbing + "sendMessage(\"" + replaceVocabs(simpleMessage).trim() + "\");", simpleMessage);
    }
    ...
    protected String replaceVocabs(String toChange) {
        ...
        while (matcher.find()) {
            String vocab = matcher.group(2);

            //Stupid spicy vocab
            if(vocab.equalsIgnoreCase("DT")) {
                toChange = matcher.replaceFirst("");
                matcher = pattern.matcher(toChange);
            } else {
                toChange = matcher.replaceFirst("%" + vocab + "%");
                matcher = pattern.matcher(toChange);
            }
        }
